(function () {
  ...
  function ensureImageView() {
    ...
    try {
      ...
      imageViewInitFailed = true;
      imageView = null;
      if (statusEl) statusEl.textContent = "Images WebGL: " + String(err && err.message ? err.message : err);
      return null;
    }
    return imageView;
  }
  ...
  if (window.PolarFieldView) {
    polarView = new PolarFieldView({ getState: function () { return state; } });
  }
  ...
  async function fetchImages() {
    ...
    try {
      ...
      requestAnimationFrame(function () {
        requestAnimationFrame(function () {
          if (currentView !== "lattice" && currentView !== "paths") return;
          if (imageView) { imageView.resize(); applyImageView(); }
        });
      });
  ...
  function applyImageView() {
    ...
    try {
      ...
      if (statusEl) statusEl.textContent = "Images rebuild: " + String(err && err.message ? err.message : err);
      return;
    }
  ...
  function placeCapsuleControls(view) {
    ...
    /* RESTORE_MARKER_12K_TRUNCATED_AT_SOURCE_BOUNDARY — full file continues in seq_20000+ */
